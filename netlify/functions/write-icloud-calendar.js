...
function buildIcsEvent({ uid, title, date, time, place }) {
  const [y, m, d] = date.split("-").map(Number);
  let dtStart, dtEnd, allDay = !time;
  if (allDay) {
    ...
    const endUtc = new Date(startUtc.getTime() + 60 * 60000); // час по умолчанию
...
exports.handler = async (event) => {
  ...
  const { action, uid, title, date, time, place } = body; // action: "create" | "delete"
  ...
  try {
    ...
    const iCalString = buildIcsEvent({ uid: eventUid, title, date, time, place });
